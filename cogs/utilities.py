...
class Utilities(commands.Cog):
    ...
    async def invite(self, ctx):
        ...
        embed = discord.Embed(
            color=0xFFFFFF, description=f'[Invite]({invite_url})', timestamp=datetime.datetime.utcnow())
    ...
    async def poll(self, ctx, *, question=None):
        ...
        embed = discord.Embed(title='Poll', color=0xFFFFFF,
                              description=question, timestamp=datetime.datetime.utcnow())
        embed.set_footer(
    ...
    async def urban(self, ctx, *, query=None):
        ...
        embed = discord.Embed(
            title='Urban Dictionary', color=0xFFFFFF, timestamp=datetime.datetime.utcnow())
    ...
    async def time_in(self, ctx, *, timezone=None):
        ...
        if utc_datetime:
            datetime_utc_obj = datetime.datetime.strptime(
                utc_datetime, format_string)
        ...
        if dst_from:
            datetime_dst_from_obj = datetime.datetime.strptime(
                dst_from.split('+')[0], "%Y-%m-%dT%H:%M:%S")
        ...
        if dst_until:
            datetime_dst_until_obj = datetime.datetime.strptime(
                dst_until.split('+')[0], "%Y-%m-%dT%H:%M:%S")
        ...
        embed = discord.Embed(
            title=f'Time in {timezone}', color=0xFFFFFF, timestamp=datetime.datetime.utcnow())
