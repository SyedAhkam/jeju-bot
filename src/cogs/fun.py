...
class Fun(commands.Cog, name='fun'):
    ...
    async def meme(self, ctx):
        ...
        embed.add_field(name='Title:', value=json_response['title'], inline=True)
        embed.add_field(name='Subreddit:', value=json_response['subreddit'], inline=True)
        embed.add_field(name='Postlink:', value=f'[Click me!]({postlink})', inline=True)

        embed.add_field(name='Author:', value=json_response['author'], inline=True)
        embed.add_field(name='Upvotes:', value=json_response['ups'], inline=True)
        embed.add_field(name='NSFW:', value=json_response['nsfw'], inline=True)

        embed.set_image(url=json_response['url'])

        embed.set_footer(text='Powered by the meme-api', icon_url=ctx.author.avatar_url)
